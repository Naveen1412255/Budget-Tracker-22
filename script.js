...
class BudgetTracker {
    ...
    setupEventListeners() {
        ...
        // Share App Modal Listeners
        document.getElementById('shareAppBtn')?.addEventListener('click', () => this.openShareAppModal());
        document.getElementById('shareAppBtnGuest')?.addEventListener('click', () => this.openShareAppModal());
        document.getElementById('closeShareAppModal')?.addEventListener('click', () => this.closeShareAppModal());
        document.getElementById('amazonShareBtn')?.addEventListener('click', () => this.shareLink('amazon'));
        document.getElementById('indusShareBtn')?.addEventListener('click', () => this.shareLink('indus'));
        document.getElementById('getappsShareBtn')?.addEventListener('click', () => this.shareLink('getapps'));
    }
    ...
    async shareLink(store) {
        const links = {
            amazon: 'https://www.amazon.com/appstore',
            indus: 'https://www.indusappstore.com/',
            getapps: 'https://global.app.mi.com/getapps'
        };
        const storeUrl = links[store];
    
        const shareData = {
            title: 'Budget Tracker App',
            text: `Check out this amazing Budget Tracker app, available on ${store}!`,
            url: storeUrl
        };
        ...
        if (navigator.share) {
            try {
                ...
                this.showToast('App shared successfully!', 'success');
            } catch (err) {
                // Handle cases where the user cancels the share dialog
                if (err.name !== 'AbortError') {
                    console.error('Share failed:', err);
                    this.showToast(`Could not share: ${err.message}`, 'error');
                }
            }
        } else {
            // Fallback for browsers that do not support the Web Share API
            this.showToast('Web Share not supported, opening link instead.', 'info');
